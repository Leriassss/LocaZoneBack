...
const HouseFeatures = require('../models/house-features');
const fs = require('fs');
...
exports.getHouseVendor = (req, res, next) => {
    const getQuery = `select * from (select namehouse,houses.id as idhouse,
                     imageUrl,quartier,voisin,prix,superficie,userId,disponibilite, typehouse,cathouse 
...
exports.getHouse = (req, res, next) => {
    if (req.query.filtres) {
        ...
        const queryLocalisation = infos.localisation ? parseInt(infos.localisation) : ''
        const queryFeatures =
        ...
        const queryTC = 'type = ' + parseInt(infos.type) + ' and categorie = ' + 
                        parseInt(infos.categorie) + ' and quartier = ' +queryLocalisation
